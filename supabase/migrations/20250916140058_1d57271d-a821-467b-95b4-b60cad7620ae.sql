-- Remove pre-insert and trigger-side duplicate suppression from notify_project_planning_correction
-- Let the edge function handle duplicates and recording instead
...
begin
  ...
  IF (OLD.status IN (4, 5) AND NEW.status = 3) THEN
    ...
    IF pg_try_advisory_xact_lock(v_lock_key) THEN
      ...
      -- Build payload only (do not pre-insert into email_notifications here)
