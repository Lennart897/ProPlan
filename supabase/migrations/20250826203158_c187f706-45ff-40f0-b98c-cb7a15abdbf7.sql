-- Prevent duplicate emails by using better advisory lock and transaction handling
...
begin
  ...
  IF (OLD.status IN (4, 5) AND NEW.status = 3) THEN
    ...
    IF v_correction_reason != '' THEN
      ...
      -- Create a unique lock key based on project ID and current timestamp (rounded to seconds)
      -- This prevents duplicate emails within the same second for the same project
      v_lock_key := ('x' || md5(NEW.id::text || date_trunc('second', now())::text))::bit(64)::bigint;
      ...
      ELSE
        RAISE LOG 'Could not acquire advisory lock for project % - duplicate prevention', NEW.project_number;
      END IF;
    ELSE
