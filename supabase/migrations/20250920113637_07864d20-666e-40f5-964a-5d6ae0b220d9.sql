-- Fix critical security vulnerabilities in RLS policies (corrected version)
...
-- Create more restrictive profile policies for project context only
...
USING (
  ...
  EXISTS (
    ...
    WHERE (mp.created_by_id = auth.uid() OR mp.created_by_name = auth.uid()::text)
    AND (mp.created_by_id = profiles.user_id OR mp.created_by_name = profiles.user_id::text)
  )
  OR EXISTS (
    SELECT 1 FROM public.project_location_approvals pla
    JOIN public.profiles p ON p.user_id = auth.uid()
    JOIN public.manufacturing_projects mp ON mp.id = pla.project_id
    WHERE (p.role LIKE 'planung%' OR p.role IN ('supply_chain', 'admin'))
    AND (mp.created_by_id = profiles.user_id OR mp.created_by_name = profiles.user_id::text)
  )
);
