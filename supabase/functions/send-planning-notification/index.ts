...
serve(async (req: Request) => {
  ...
  try {
    ...
    const emailPromises = recipientEmails.map(async (email) => {
      ...
      console.log(`Planning notification sent via SendGrid to: ${email}, status:`, response.status);
    });

    await Promise.all(emailPromises);
    console.log("Planning notification emails sent via SendGrid", { id, emailsSent: recipientEmails.length, recipients: recipientEmails });
