-- Update notify_project_approval to also send emails when planning users approve projects
-- Remove the restriction that prevented emails from planning users
...
begin
  -- Send email when status changes to approved (5) from any non-approved status
  IF NEW.status = 5 AND (OLD.status IS NULL OR OLD.status != 5) THEN
    ...
    IF pg_try_advisory_xact_lock(v_lock_key) THEN
      ...
      RAISE LOG 'Sending project approval email for project % (approval by %)', NEW.project_number, v_current_user_role;
