...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Send payload with toRecipients array for Make webhook
    const payload = {
      subject: `ProPlan - Neues Projekt #${project_number}: ${artikel_bezeichnung}`,
      body: professionalEmailContent,
      toRecipients: toRecipients, // Required array format for Make webhook
      assigned_to: assignedTo, // Backup array of email addresses
      metadata: {
        type: "project",
