-- Fix the trigger to remove the invalid supabase_functions.invoke call
...
AS $function$
declare
  v_url text := 'https://rhubaybwftyypfbiuoyc.supabase.co/functions/v1/send-project-correction-email';
  v_headers jsonb := jsonb_build_object('Content-Type','application/json', 'Authorization', 'Bearer ' || current_setting('request.jwt.claims', true)::json->>'anon_key');
  ...
  IF OLD.status = 3 AND NEW.status = 2 THEN
    ...
    IF v_user_role = 'supply_chain' THEN
      ...
      IF pg_try_advisory_xact_lock(v_lock_key) THEN
        ...
        -- Use net.http_post instead of the invalid supabase_functions.invoke
