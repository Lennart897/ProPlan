-- Erweitere die Funktion für Projektersteller - erlaubt Absage in jedem Status
...
BEGIN
  ...
  -- Check if user is creator (either by UUID or string) - allow cancellation in any status except already cancelled (6) or completed (7)
  RETURN (
    (project_creator_id = user_uuid OR project_creator_name = current_user_uuid_text) 
    AND project_status NOT IN (6, 7) -- Don't allow cancelling already cancelled or completed projects
  );
END;
$function$;